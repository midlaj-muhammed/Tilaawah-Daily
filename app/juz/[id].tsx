...
export default function JuzReader() {
    ...
    useEffect(() => {
        const loadJuz = async () => {
            if (!id) return;
            setLoading(true);
            const result = await quranService.getJuz(parseInt(id as string), preferredTranslation);
            if (result && result.ayahs.length > 0) {
                setAyahs(result.ayahs);
                updateProgress({
                    lastReadSurahId: result.ayahs[0].surahId,
                    lastReadAyahNumber: result.ayahs[0].ayahNumber
                });
            }
            setLoading(false);
            animateAyah();
    ...
    const playAudio = async () => {
        try {
            ...
            const surahId = ayahs[currentIndex]?.surahId || 1;
    ...
    const handleBookmarkToggle = () => {
        ...
        const surahId = currentAyah.surahId;
    ...
    if (ayahs.length === 0) {
        return (
            <View style={styles.errorContainer}>
                <LinearGradient colors={['#1A103D', '#2D1B69']} style={StyleSheet.absoluteFillObject} />
                <Text style={{ color: '#FFF' }}>Juz not found</Text>
    ...
    const isBookmarked = bookmarks.some(b => b.surahId === currentAyah?.surahId && b.ayahNumber === currentAyah?.ayahNumber);
            ...
            <SafeAreaView style={styles.safeArea}>
                ...
                <View style={styles.header}>
                    ...
                    <View style={styles.headerCenter}>
                        <Text style={styles.headerTitle}>Juz {id}</Text>
                        ...
                        <ScrollView contentContainerStyle={styles.ayahScroll} showsVerticalScrollIndicator={false}>
                            ...
                            <View style={styles.ayahHeader}>
                                ...
                                <View style={styles.ayahHeaderCenter}>
                                    <Text style={styles.ayahSurahName}>{currentAyah.surahEnglishName || `Surah ${currentAyah.surahId}`}</Text>
